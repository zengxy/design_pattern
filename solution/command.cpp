...
class Command;
class File;
typedef void (File::* ActionCommand )();
...
public:
    ...
    void unarchive()
    {
        cout << "unarchive " << name_ << endl;
    }
    void compress()
    {
        cout << "compress "  << name_ << endl;
    }
    void transfer()
    {
        cout << "transfer "  << name_ << endl;
    }
private:
    char  name_[30];
};
...
public:
    Command( File* f, Action a )
    {
        receiver = f;
        if(a==transfer)
            commandFunc=&File::transfer;
        else if(a==unarchive)
            commandFunc=&File::unarchive;
        else if(a==compress)
            commandFunc=&File::compress;
    }
    void execute()
    {
        (receiver->*commandFunc)();
    }

private:
    File*   receiver;
    ActionCommand commandFunc;
};
